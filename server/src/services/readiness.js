...
export async function readiness({ campusSlug = 'upes-bidholi' } = {}) {
  ...
  const items = [
    ...
    item('Administrators', 'second_admin', 'A second administrator (Anushka) is active', admins.other_admins > 0, { how: admins.invites ? 'An invitation is waiting for sign-in.' : 'Needs her real UPES email: Administrators → Invite administrator.', blocking: false }),
